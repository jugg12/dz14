...
import history__def from "history";
import jwt_decode from "jwt-decode";
import Header from "../glavnaya/Header";
import Registraciya from "./registraciya/registraciya";
...
const Vhod = () =>{
  ...
  .then((res)=>{  
    if(res.data.length===0){
      const errorClass=document.querySelector(".OshibkaVvoda")
      value.login=""
      value.password=""
          ...
          <div className="Osnovnoe">
            ...
            <p className="Primech"> публиковать свои объявления</p>
              <div className="LogPass">
                <div className="LOGIN">
                  <Field className={((touched.login && errors.login)||(values.login===""))? "login__error":"login"} onChange={handleChange} onBlur={handleBlur} validate={validateLogin} name = "login" type="text" value={values.login} placeholder="Логин"/>
                ...
                <div className="PASSWORD">
                  <Field className={((touched.password && errors.password)||(values.password===""))?"password__error":"password"} onChange={handleChange} onBlur={handleBlur} validate={validatePassword} name = "password" value={values.password} id="password" type="password" placeholder="Пароль" />
            ...
            <div className="Zabil">
                ...
                <Link to="/registraciya/registraciya" className="zabpas">Забыли пароль?</Link>
            </div>
            <div className={("OshibkaVvoda fix" && ((touched.login&&errors.login)||(touched.password && errors.password)))? "OshibkaVvoda fix":"OshibkaVvoda"}>
              <div className="">
                <p className="OshibkaVvodaText">Неверный логин или пароль</p>
              </div>
              <div className="">
            ...
            <div className="last2">
              ...
              <Link to="/registraciya/registraciya" className="Registration zabpas"> Создайте аккаунт</Link>
            </div>
          </div>
        </Form> 
        )}
